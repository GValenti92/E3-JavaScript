...
//Adquiero  el id de la última pizza desde el local storage
...
function leerForm(e) {
  ...
  if (buscaPizza(dato) != 0) {
    // si encuentró la pizza

    imprimePizza(pizzaOk);

    //Guardo la  pizza en localstorage y la reemplazo por  la anterior.
    localStorage.setItem("ultimaPizza", JSON.stringify(pizzaOk));
  } else if (ingresoAlgo) {
    //Si no encuentró la pizza
    errorDisplay("El número de pizza ingresado no existe");
  } else {
    //Error al no ingresar nada  o número inexistente:
